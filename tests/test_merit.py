...
def test_calc_position_error():
    ...
    position_error_reversed = calc_position_error(test_image_p2, test_image_p1, method="GREIT")
    ...
    assert position_error_euclidean_p2_flipped == correct_position_error_euclidean_p2_flipped
...
def test_calc_shape_deformation():
    ...
    np.testing.assert_almost_equal(shape_deformation_circular, correct_shape_deformation_circular)


def test_classify_target_and_background():
    target, _ = classify_target_and_background(test_image_2, conductive_target=True)
    target_negative, _ = classify_target_and_background(test_image_3, conductive_target=False)
    correct_target = np.array([[False, False, False, False, False, False, False],
