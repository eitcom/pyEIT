...
mesh_obj, el_pos = mesh.create(len(p_fix), 
                               ...
                               subdivideregions = [ np.array([[-100e-6, 0], [100e-6, 50e-6]]), ])
...
for right_edge in np.arange(-50e-6,100e-6,10e-6):
    print('right_edge',right_edge)
    #print('bead diameter',bead_diameter)
    #print('bead height',bead_height)
    anomaly = [{"bbox": np.array([[-100e-6, 4e-6], [right_edge, 10e-6]]), "perm": 0.25},
                {"bbox": np.array([[-100e-6, 10e-6], [right_edge-50e-6, 35e-6]]), "perm": 0.1},]
    ...
    ds = eit.gn(f1.v, lamb_decay=0.1, lamb_min=1e-5, maxiter=10, verbose=True)
