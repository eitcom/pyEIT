...
slicecol = 220
for xmin in range(281,400,4):  #range(310,400,5):  #range(280,340,5): [300,]: #[100,200,300,420]:
    #xmin,xmax=[100,200]
    xmax = 511
    ymin,ymax=[0,255]
    ...
    #        minerva_data[:,2][ect_gap==spacing] = minerva_data[:,2][ect_gap==spacing] + min(meas_mat[:,2][ect_gap==spacing])
        ...
        fig.savefig('data/reconstruction8_xmin_%u.pdf' % (xmin,))
        
        with open('data/reconstruction8_xmin_%u.npy' % (xmin,), 'wb') as f:
...
slicedata_files = sorted(glob.glob('data/reconstruction8_xmin_*.npy'))
...
for myfile in slicedata_files:
    ...
    fig.savefig('data/playback_reconstruction8_xmin_%u.pdf' % (xmin,))
