...
anomaly = [{'x': 0.5, 'y': 0.5, 'd': 0.2, 'perm': 10},
           {'x': -0.2, 'y': -0.2, 'd': 0.4, 'perm': 20}]
ms0 = wrapper.set_perm(mesh_obj, anomaly=anomaly, background=1.)

anomaly = [{'x': 0.5, 'y': 0.5, 'd': 0.2, 'perm': 20},
           {'x': -0.2, 'y': -0.2, 'd': 0.4, 'perm': 10}]
ms1 = wrapper.set_perm(mesh_obj, anomaly=anomaly, background=1.)
...
im = ax.tripcolor(pts[:, 0], pts[:, 1], tri, np.real(node_ds), edgecolor='k',
                  shading='flat', alpha=0.8, cmap=plt.cm.RdBu)
