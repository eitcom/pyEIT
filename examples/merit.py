...
from pyeit.visual.plot import create_mesh_plot_2, create_plot, create_image_plot, create_layered_image_plot
...
def main():
    ...
    image = model_inverse_uv({"node": sim_mesh.node[:, :2], "element": sim_mesh.element}, resolution=(1000, 1000))
    ...
    out_circular = {}
    figs_circular = merit.calc_greit_figures_of_merit(render, recon_render, out=out_circular)

    out_non_circular = {}
    figs_non_circular = merit.calc_greit_figures_of_merit(render, recon_render, circular=False, out=out_non_circular)
    ...
    create_layered_image_plot(
        ax,
        (render.T, out_circular["shape_deformation"]["shape_deformation_target"].T,
    ...
    create_layered_image_plot(
        ax,
        (
        render.T, out_circular["ringing"]["ringing_target"].T, out_circular["ringing"]["opposite_outside_positions"].T),
    ...
    create_layered_image_plot(
        ax,
        (render.T, out_non_circular["shape_deformation"]["shape_deformation_target"].T,
    ...
    create_layered_image_plot(
        ax,
        (render.T, out_non_circular["ringing"]["ringing_target"].T,
